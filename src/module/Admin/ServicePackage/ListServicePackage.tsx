...
const ListServicePackage = () => {
    ...
    const { data, isLoading, isError, isRefetching } = useSearchServicePackageForAdQuery({
                ...
                Cell: ({ row }) => {
                    return (
                        <p className="line-clamp-1 max-w-[300px]">
                            {row.original.serviceItems.map((item, index) => (
                                item.serviceName + (index + 1 === row.original.serviceItems.length ? "" : ",")
                            ))}
                        </p>
                    )
                }
